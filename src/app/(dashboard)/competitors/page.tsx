...
export default function FiveWayCompetitorArenaPage() {
  const { competitors, recommendations, addEntity, entityType, startNewComparisonPrompt, primaryEntity } = useTenant();
    ...
    <div className="space-y-8 pb-12">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-bold text-teal-light uppercase tracking-wider">
              {entityType === "individual" ? "5-Way Individual & Constituency Leadership Benchmarking" : "5-Way Corporate Benchmarking"}
            </span>
          </div>
          <h1 className="text-2xl font-extrabold tracking-tight">Five-Way {entityType === "individual" ? "Candidate & Leader" : "Competitor"} Arena</h1>
          <p className="text-xs text-slate-200 mt-1 max-w-xl">
            {entityType === "individual"
              ? "Comprehensive multi-axis benchmark tracking Local Standing, Support to Public, Popularity, Public Status, and Service to Society & Constituency across all 5 leaders."
              : "Simultaneous multi-axis analysis comparing 5 corporate brands across Share of Voice, sentiment momentum, and market whitespace."}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-base font-bold text-slate-900">
                {entityType === "individual" ? "5-Pillar Public Standing & Service Radar" : "5-Axis Capability Radar"}
              </h2>
              <p className="text-xs text-slate-400">
                {entityType === "individual"
                  ? "Comparing Local Standing, Support to Public, Popularity, Public Status & Constituency Service"
              ...
              <RadarChart cx="50%" cy="50%" outerRadius="75%" data={radarMetrics}>
                ...
                {competitors.slice(0, 5).map((comp, idx) => (
                  <Radar
                    ...
                    fillOpacity={comp.isPrimary ? 0.45 : 0.15}
                  />
                ))}
        ...
        <div className="bg-card p-6 rounded-3xl border border-slate-200/80 shadow-ios">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-base font-bold text-slate-900">Popularity & Public Mindshare Share of Voice</h2>
              <p className="text-xs text-slate-400">Percentage distribution across 5 monitored {entityType === "individual" ? "leaders" : "brands"}</p>
            </div>
          </div>
